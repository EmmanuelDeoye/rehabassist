// js/gaitresult.js - Gait Analysis Report Editor with public/private sharing
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const gaitPatientNameSpan = document.getElementById('gaitPatientName');
    const gaitDateSpan = document.getElementById('gaitDate');
    const gaitViewSpan = document.getElementById('gaitView');
    const gaitLastEditedSpan = document.getElementById('gaitLastEdited');
    ...
    const closeGaitBtn = document.getElementById('closeGaitBtn');
    ...
    if (!historyId) {
        ...
        editor.innerHTML = '<div class="loading-editor"><i class="fas fa-exclamation-circle"></i> No gait analysis found. Please generate an analysis first.</div>';
        return;
    }
    ...
    function updateSaveStatus(status, isError = false) {
        ...
        if (isError) {
            ...
            saveStatusSpan.style.color = 'var(--gait-accent, #009688)';
        }
    }
    ...
    async function togglePublic(event) {
        ...
        if (!currentUser || !historyId || !currentIsOwner) {
            showToast('You are not the owner of this report.', 'error');
        ...
        try {
            ...
            await firebase.database().ref(`history/${currentUser.uid}/gaitHistory/${historyId}`).update(updates);
            ...
            if (isChecked) {
                ...
                showToast('✅ Report is now public. Anyone with the link can view it.', 'success');
            } else {
                await firebase.database().ref(`publicAnalysis/${historyId}`).remove();
                showToast('🔒 Report is now private.', 'success');
            }
        } catch (error) {
    ...
    async function saveToFirebase() {
        ...
        if (!currentIsOwner) {
            showToast('You cannot edit a shared report.', 'error');
            return;
        }
        ...
        try {
            ...
            if (gaitLastEditedSpan) {
                gaitLastEditedSpan.textContent = analysisData?.lastEditedDate || new Date().toLocaleString();
            }
        } catch (error) {
    ...
    async function loadAnalysis() {
        try {
            ...
            if (currentUser) {
                const snapshot = await firebase.database().ref(`history/${currentUser.uid}/gaitHistory/${historyId}`).once('value');
                data = snapshot.val();
                if (data) {
                    ownerId = currentUser.uid;
                    currentIsOwner = true;
            ...
            if (!data || data.contentType !== 'gait') {
                throw new Error('Gait analysis not found or private');
            }
            
            analysisData = data;
            
            // Update gait-specific metadata
            if (gaitPatientNameSpan) gaitPatientNameSpan.textContent = data.patientName || data.fileName || 'Unnamed Patient';
            if (gaitDateSpan) gaitDateSpan.textContent = data.date || new Date().toLocaleDateString();
            if (gaitViewSpan) gaitViewSpan.textContent = data.view || 'Not specified';
            if (gaitLastEditedSpan) gaitLastEditedSpan.textContent = data.lastEditedDate || data.date || '-';
            ...
            if (!currentIsOwner) {
                ...
                showToast('📖 You are viewing a shared gait report (read-only).', 'info', 4000);
            } else {
            ...
            showToast('Failed to load gait analysis', 'error');
            editor.innerHTML = '<div class="loading-editor"><i class="fas fa-exclamation-circle"></i> Error loading gait report. It may be private or does not exist.</div>';
        }
    }
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const patientName = analysisData?.patientName || 'Unnamed Patient';
                
                const doc = new docx.Document({
                    sections: [{
                        children: [
                            new docx.Paragraph({ 
                                text: `Gait Analysis Report - ${patientName}`, 
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', () => {
            ...
            const opt = {
                ...
                filename: `Gait_Analysis_${analysisData?.patientName || 'report'}_${Date.now()}.pdf`,
    ...
    if (printBtn) {
        printBtn.addEventListener('click', () => {
            ...
            printWindow.document.write(`
                <!DOCTYPE html>
                <html>
                <head><title>Gait Analysis Report</title>
                <style>
                ...
                <body>
                    <h1>Gait Analysis Report - ${analysisData?.patientName || 'Unnamed Patient'}</h1>
                    <p><strong>Generated:</strong> ${new Date().toLocaleString()}</p>
                    <p><strong>View:</strong> ${analysisData?.view || 'N/A'}</p>
                    <hr>
                    ${editor.innerHTML}
                    <hr>
                    <p style="font-size:0.8rem;">Generated by rehablix Gait Monitor</p>
                </body></html>
            `);
    ...
    if (closeGaitBtn) {
        closeGaitBtn.addEventListener('click', () => {
    ...
    if (shareEmailBtn) {
        shareEmailBtn.addEventListener('click', () => {
            const subject = encodeURIComponent(`Gait Analysis: ${analysisData?.patientName || 'Report'}`);
            const body = encodeURIComponent(`Check out this gait analysis: ${shareLink.value}`);
    ...
    if (shareWhatsAppBtn) {
        shareWhatsAppBtn.addEventListener('click', () => {
            const text = encodeURIComponent(`Check out this gait analysis: ${shareLink.value}`);
