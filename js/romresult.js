// js/romresult.js - ROM Analysis Report Editor with public/private sharing
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const romJointNameSpan = document.getElementById('romJointName');
    const romDateSpan = document.getElementById('romDate');
    const romModeSpan = document.getElementById('romMode');
    const romFrameCountSpan = document.getElementById('romFrameCount');
    const romLastEditedSpan = document.getElementById('romLastEdited');
    ...
    const closeRomBtn = document.getElementById('closeRomBtn');
    ...
    if (!historyId) {
        ...
        editor.innerHTML = '<div class="loading-editor"><i class="fas fa-exclamation-circle"></i> No analysis found. Please generate a ROM analysis first.</div>';
        return;
    }
    ...
    function updateSaveStatus(status, isError = false) {
        ...
        if (isError) {
            ...
            saveStatusSpan.style.color = 'var(--rom-accent)';
        }
    }
    ...
    async function saveToFirebase() {
        ...
        try {
            ...
            if (romLastEditedSpan) {
                romLastEditedSpan.textContent = analysisData?.lastEditedDate || new Date().toLocaleString();
            }
        } catch (error) {
    ...
    async function loadAnalysis() {
        try {
            ...
            if (!data) {
                throw new Error('ROM analysis not found or private');
            }
            
            analysisData = data;
            
            // Update ROM-specific metadata
            if (romJointNameSpan) romJointNameSpan.textContent = data.fileName || 'ROM Analysis';
            if (romDateSpan) romDateSpan.textContent = data.date || new Date().toLocaleDateString();
            if (romModeSpan) romModeSpan.textContent = data.assessmentMode === 'full' ? 'Full Assessment' : 'Isolate Joint';
            if (romFrameCountSpan) romFrameCountSpan.textContent = `${data.frameCount || 0} frames`;
            if (romLastEditedSpan) romLastEditedSpan.textContent = data.lastEditedDate || data.date || '-';
            ...
            if (!currentIsOwner) {
                ...
                showToast('📖 You are viewing a shared ROM report (read-only).', 'info', 4000);
            } else {
            ...
            showToast('Failed to load ROM analysis', 'error');
            editor.innerHTML = '<div class="loading-editor"><i class="fas fa-exclamation-circle"></i> Error loading ROM report. It may be private or does not exist.</div>';
        }
    }
            ...
            try {
                ...
                const doc = new docx.Document({
                    sections: [{
                        children: [
                            new docx.Paragraph({ 
                                text: analysisData?.fileName || 'ROM Analysis Report', 
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', () => {
            ...
            const opt = {
                ...
                filename: `ROM_Analysis_${Date.now()}.pdf`,
    ...
    if (printBtn) {
        printBtn.addEventListener('click', () => {
            ...
            printWindow.document.write(`
                <!DOCTYPE html>
                <html>
                <head><title>ROM Analysis Report</title>
                <style>
                ...
                <body>
                    <h1>${analysisData?.fileName || 'ROM Analysis Report'}</h1>
                    <p><strong>Generated:</strong> ${new Date().toLocaleString()}</p>
                    <hr>
                    ${editor.innerHTML}
                    <hr>
                    <p style="font-size:0.8rem;">Generated by rehablix ROM Analyzer</p>
                </body></html>
            `);
    ...
    if (closeRomBtn) {
        closeRomBtn.addEventListener('click', () => {
            window.location.href = 'rom.html';
        });
    }
    ...
    if (shareEmailBtn) {
        shareEmailBtn.addEventListener('click', () => {
            const subject = encodeURIComponent(`ROM Analysis: ${analysisData?.fileName || 'Report'}`);
            const body = encodeURIComponent(`Check out this ROM analysis: ${shareLink.value}`);
    ...
    if (shareWhatsAppBtn) {
        shareWhatsAppBtn.addEventListener('click', () => {
            const text = encodeURIComponent(`Check out this ROM analysis: ${shareLink.value}`);
