...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function loadStudySets() {
    if (!currentUser) return;
    const snap = await database.ref(`study/${currentUser.uid}/sets`).once('value');
    studySets = snap.val() || {};
  }
...
Generate exactly ${flashcardCount} flashcards and exactly ${quizCount} quiz questions. Flashcards should test recall of specific facts/definitions. Quiz questions should test understanding, not just memorization. Every topic value must exactly match one of the strings in "topics".`;
      ...
      for (const topicName of parsed.topics) {
        ...
        if (!subjects[subjectId].topics || !subjects[subjectId].topics[topicId]) {
          await database.ref(`subjects/${currentUser.uid}/${subjectId}/topics/${topicId}`).set({ name: topicName, masteryScore: 50, timesReviewed: 0, lastActivity: new Date().toISOString() });
        }
      }
      await loadSubjects();
      ...
      const setRef = database.ref(`study/${currentUser.uid}/sets`).push();
      const setRecord = {
        subjectId,
  ...
  async function applyGrade(card, grade) {
    ...
    await database.ref(`study/${currentUser.uid}/sets/${card.setId}/flashcards/${card.cardIndex}`).update(update);
